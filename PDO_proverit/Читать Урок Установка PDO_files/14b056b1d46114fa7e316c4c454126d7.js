...
function youtubeInit() {
	//	init all players
	if (jQuery("div[data-init=youtubeVideo]").size()) {
		youtubeLoadApi();
	}
}
...
function onYouTubeIframeAPIReady() {
	//	init all players
	jQuery("div[data-init=youtubeVideo]").each(function() {
		jqDiv = jQuery(this);
		//	name(key) of player
		id = jqDiv.attr('id');
		width = jqDiv.attr('data-width');
		width = width ? width : 640;
		height = jqDiv.attr('data-height');
		height = height ? height : 390;
//		videoId = jqDiv.attr('data-id');
		if (typeof(id) == 'undefined') {
			//	error field name unknown
			alert('error: field id unknown');
		}
		else {
...
/*			not found		*/
/*
//	base class for nano-object
function nanoApp() {
}
//	create nano-object
nano = new nanoApp();
//	base constants
NANO_URL = 'http://ramech.net/';

nanoApp.prototype.url = function(uri) {
	return NANO_URL + uri;
}
*/
...
//	if(typeof jQuery === 'undefined'){
		//document.write('<script type="text/javascript" src="http://ramech.net/plugins/jquery/jquery-1.3.2.min.js"></script>');
	//}
//}
...
function nanoButtonDemand(key, el){
	message = 'Нажмите "Да" для подтверждения востребованности информации.\n\
Введите имейл (не обязательно), если вы желаете получить извещение об ответе на ваш запрос.\n\
Спасибо.\n\
';
...
/**
	Показывает анимацию AJAX-запроса
*/
function ajaxAnimationShow( elementID) {
...
/**
	Скрывает анимацию AJAX-запроса
*/
function ajaxAnimationHide( elementID) {
	elementID = elementID.replace( /\[/, '\\[' );
	elementID = elementID.replace( /\]/, '\\]' );
	$("#"+elementID+"").hide();
}
...
/**
	Включает анимацию, для отображенного индикатора
*/
...
/**
	Выключает анимацию, для отображенного индикатора
*/
function ajaxAnimationStop( elementID) {
	elementID = elementID.replace( /\[/, '\\[' );
	elementID = elementID.replace( /\]/, '\\]' );
	$("#"+elementID+"").html('<img src="/images/loading_off.gif" />');
} 
...
/*			not found		*/
jQuery(function(){
	jQuery('#lessonContent img').each(function(){
		jIMG = jQuery(this);
		if (jIMG.css('float') == 'left') {
			jIMG.addClass('alignLeft');
		}
		if (jIMG.css('float') == 'right') {
			jIMG.addClass('alignRight');
		}
		if (jIMG.attr('data-normal') || jIMG.attr('data-original')) {
			jIMG.addClass('clickabled');
			jIMG.click(function(){
				jThis = jQuery(this);
				if (jThis.attr('data-normal')) {
					location.href = jThis.attr('data-normal');
				}
				else if (jThis.attr('data-original')) {
					location.href = jThis.attr('data-original');
				}
			});
		}
	});
	//	добавление анкоров для <h2>
	jQuery('#lessonContent h2').each(function(){
		jH2 = jQuery(this);
		text = jH2.text();
		jH2.before('<a id="anchor' + text + '"></a>');
	});
});
/*			not found		*/
/*			not found		*/
